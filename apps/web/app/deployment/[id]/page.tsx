...
export default function DeploymentDetails() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const filesRes = await fetch(
        `${API_BASE_URL}/deployments/${deploymentId}/files`
      )
      if (filesRes.ok) {
  ...
  useEffect(() => {
    ...
    const logEventSource = new EventSource(
      `${API_BASE_URL}/logs/${deploymentId}/stream`
    )
    ...
    const statusEventSource = new EventSource(
      `${API_BASE_URL}/deployments/stream`
    )
    ...
    <div className="min-h-screen bg-zinc-950 p-6 font-sans text-zinc-50 sm:p-20">
      <div className="mx-auto max-w-4xl space-y-10">
        ...
        <div className="flex flex-col justify-between gap-6 sm:flex-row sm:items-center">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="mt-1 flex items-center gap-2">
                ...
                <button
                  onClick={() =>
                    router.push(`/project/${deployment.projectId}`)
                  }
                  className="cursor-pointer font-mono text-[10px] text-zinc-500 transition-colors hover:text-zinc-300"
                >
          ...
          {/* --- STATUS BADGE --- */}
          <div
            className={`flex items-center gap-2 rounded-full border px-4 py-1.5 text-[10px] font-bold tracking-widest uppercase shadow-sm ${
              deployment.status === "ready"
                ? "border-green-500/20 bg-green-500/10 text-green-500"
                : deployment.status === "failed"
                  ? "border-red-500/20 bg-red-500/10 text-red-500"
                  : "animate-pulse border-blue-500/20 bg-blue-500/10 text-blue-500"
            }`}
          >
        ...
        >
          ...
          <AccordionItem
            ...
            className={`overflow-hidden rounded-xl border border-zinc-900 bg-zinc-900/20 ${deployment.status === "queued" && "opacity-30"}`}
          >
            ...
            <AccordionContent className="px-6 pb-6">
              <div className="h-[400px] overflow-y-auto rounded-lg border border-zinc-800 bg-black/40 p-4 font-mono text-[11px] leading-relaxed">
                {logs.length === 0 ? (
                  <div className="py-6 text-center text-zinc-700 italic">
                    No logs available for this build phase.
                  </div>
                ) : (
                  logs.map((log, i) => (
                    <div key={i} className="mb-0.5 font-mono text-zinc-400">
          ...
          <AccordionItem
            ...
            className={`overflow-hidden rounded-xl border border-zinc-900 bg-zinc-900/20 ${(deployment.status !== "ready" || files.length === 0) && "opacity-30 transition-opacity"}`}
          >
            <AccordionTrigger className="group px-6 py-4 hover:no-underline">
              <div className="flex items-center gap-3">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-1 gap-2">
                  {files.map((file, i) => (
                    <div
                      key={i}
                      className="flex items-center justify-between rounded-lg border border-zinc-800/50 bg-zinc-950/40 p-3 text-[11px] transition-colors hover:border-zinc-700"
                    >
                      <span className="max-w-[500px] truncate font-mono text-zinc-300">
                        {file.key}
                      </span>
          ...
          <AccordionItem
            ...
            className={`overflow-hidden rounded-xl border border-zinc-900 bg-zinc-900/20 ${deployment.status !== "ready" && "opacity-30"}`}
          >
            <AccordionTrigger className="group px-6 py-4 hover:no-underline">
              <div className="flex items-center gap-3">
                <Globe className="size-4 text-zinc-500" />
                <span className="text-sm font-semibold text-zinc-300">
                  Custom Assigned Domain
                </span>
              </div>
            </AccordionTrigger>
            <AccordionContent className="px-6 pb-6">
              <div
                className="group/link flex cursor-pointer items-center justify-between rounded-lg border border-zinc-800/50 bg-zinc-950/40 p-4 transition-colors hover:border-zinc-700"
                onClick={() =>
                  window.open(
                    `http://${deployment.id}.localhost:8080`,
                    "_blank"
                  )
                }
              >
                <div className="flex items-center gap-3">
                  <Globe className="size-4 text-zinc-500" />
                  <span className="text-sm font-medium text-zinc-300 transition-colors group-hover/link:text-zinc-100">
                    {deployment.id}.localhost:8080
                  </span>
                </div>
                <ExternalLink className="size-3.5 text-zinc-700 transition-colors group-hover/link:text-zinc-500" />
              </div>
            </AccordionContent>
          </AccordionItem>
        </Accordion>
