...
app.use(async (req, res, next) => {
  ...
  if (UUID_REGEX.test(subdomain)) {
    ...
    try {
      ...
      if (result.rowCount === 0)
        return next(new ApiError(404, "Deployment not found."))
  ...
  // MODE 2: Requesting the main project subdomain
    ...
    try {
      ...
      if (result.rowCount === 0)
        return next(
          new ApiError(404, "No successful deployment found for this project.")
        )
      const row = result.rows[0]
      if (row && typeof row.id === "string") {
        deploymentId = row.id
        ...
        return next(
          new ApiError(500, "Internal error: Deployment ID not found.")
        )
      }
    } catch (err) {
      return next(err)
  ...
  return proxy.web(req, res, {}, (err: any) => {
    ...
    next(
      new ApiError(502, `Proxy error: Connection to S3 failed. ${err.message}`)
    )
  })
})
