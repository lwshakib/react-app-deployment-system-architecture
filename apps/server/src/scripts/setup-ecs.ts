...
async function autoPushDockerImage(repositoryUri: string) {
  ...
  execSync(`docker login --username AWS --password ${password} ${endpoint}`, {
    stdio: "inherit",
  })
  ...
  execSync(`docker build -t build-container:latest ${buildContext}`, {
    stdio: "inherit",
  })

  // Tag and Push the image
  logger.info(`🏷️ Tagging local image...`)
  execSync(`docker tag build-container:latest ${repositoryUri}:latest`, {
    stdio: "inherit",
  })
...
async function setupECS() {
  ...
  try {
    ...
    const executionRoleArn = await getOrCreateRole(
      "ReactAppDeployTaskExecutionRole",
      ecsAssumeRolePolicy
    )
    ...
    const taskRoleArn = await getOrCreateRole(
      "ReactAppDeployTaskRole",
      ecsAssumeRolePolicy
    )
    await iamClient.send(
      new AttachRolePolicyCommand({
    ...
    let repositoryUri = ""
    try {
      const ecrRes = await ecrClient.send(
        new DescribeRepositoriesCommand({
          repositoryNames: ["build-container"],
        })
      )
      ...
      if (error.name === "RepositoryNotFoundException") {
        ...
        const createEcr = await ecrClient.send(
          new CreateRepositoryCommand({ repositoryName: "build-container" })
        )
    ...
    const clusterRes = await ecsClient.send(
      new CreateClusterCommand({ clusterName: "react-app-deploy-cluster" })
    )
    ...
    await cwLogsClient.send(
      new PutRetentionPolicyCommand({ logGroupName, retentionInDays: 7 })
    )
    ...
    const taskDefRes = await ecsClient.send(
    ...
    logger.info(
      `✅ The infrastructure and ECR image are fully deployed and ready for use!`
    )
  } catch (error) {
