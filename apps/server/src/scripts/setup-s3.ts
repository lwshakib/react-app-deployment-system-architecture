...
if (!region || !bucketName) {
  logger.error(
    "❌ Missing AWS environment variables (AWS_REGION, S3_BUCKET_NAME)."
  )
  process.exit(1)
}
...
async function setupS3() {
  ...
  try {
    ...
    try {
      ...
      exists = true
      logger.info(
        `ℹ️ Bucket ${bucketName} already exists. Proceeding to update configuration.`
      )
    } catch (err: any) {
    ...
    logger.info(
      `🎉 S3 setup complete! Your web files will be publicly accessible.`
    )
  } catch (error) {
