...
import clickHouseService from "./clickhouse.services.js"
import {
  KAFKA_BROKER,
  KAFKA_CA_CERT,
  KAFKA_CLIENT_ID,
...
import eventBus from "./event-bus.services.js"
...
class KafkaService {
  ...
  constructor() {
    ...
    if (!broker || !username || !password || !clientId) {
      throw new Error(
        "❌ Kafka environment variables are missing. Infrastructure cannot be initialized."
      )
    }
  ...
  async listenBatch(topic: string, groupId: string, onBatch: any) {
    try {
      ...
      logger.error(
        `❌ Failed to start Kafka batch consumer for topic ${topic}:`,
        error
      )
      throw error
    }
  ...
  async initLogConsumer() {
    await this.listenBatch(
      "container-logs",
      "api-server-logs-consumer",
      async ({ batch, heartbeat, resolveOffset }: any) => {
        const logEntries = []
        const validMessages = []
        ...
        if (logEntries.length > 0) {
          try {
            ...
            logger.info(
              `📝 Log Consumer: Batched ${logEntries.length} logs to ClickHouse.`
            )
          } catch (err) {
            logger.error(
              "❌ Kafka Log Consumer: ClickHouse Insertion Failed:",
              err
            )
      ...
      async ({ batch, heartbeat, resolveOffset }: any) => {
        for (const message of batch.messages) {
          ...
          try {
            ...
            if (!status) {
              logger.warn(
                `⚠️ Status Consumer: Missing status for deployment ${DEPLOYMENT_ID}`
              )
              resolveOffset(message.offset)
              continue
            }
            ...
            if (res.rowCount === 0) {
              logger.warn(
                `⚠️ Status Consumer: No deployment found with ID ${DEPLOYMENT_ID}`
              )
            } else {
              logger.info(
                `🔔 Status Consumer: Updated deployment ${DEPLOYMENT_ID} to ${status}`
              )
            }
            // Mark as processed
            resolveOffset(message.offset)
  ...
  async disconnect() {
    if (this.producer)
      await this.producer.connect().then(() => this.producer?.disconnect())
