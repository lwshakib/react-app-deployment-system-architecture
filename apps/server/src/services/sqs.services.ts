...
class SQSService {
  ...
  constructor() {
    ...
    if (!region || !accessKeyId || !secretAccessKey || !queueUrl) {
      throw new Error(
        "❌ SQS environment variables are missing. SQS service cannot be initialized."
      )
    }
        ...
        if (response.Messages && response.Messages.length > 0) {
          for (const message of response.Messages) {
            if (message.Body) {
              ...
              logger.info(
                `📥 Received deployment request from SQS: ${payload.deploymentId}`
              )
              ...
              try {
                if (isDev) {
                  ...
                  logger.info(
                    `🏠 Development Mode: Starting local Docker container build...`
                  )
                ...
                logger.error(
                  "❌ Failed to execute build task, updating status to FAILED:",
                  error
                )
                await postgresService.query(
