...
import { S3_REVERSE_PROXY_URL } from "../envs.js"
...
// Helper utilities
import { generateUniqueSubDomain } from "../utils/project.utils.js"
...
export const getDeployments = asyncHandler(
  ...
  `
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, data, "Deployments fetched successfully"))
  }
)
...
export const getDeploymentById = asyncHandler(
  ...
  `
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, data, "Deployment fetched successfully"))
  }
)
  ...
  async (req: Request, res: Response) => {
    ...
    if (effectiveProjectId) {
      ...
      if (!repo || !url)
        throw new ApiError(
          400,
          "Repo name and Git URL are required for new projects"
        )
    ...
    return res
      .status(201)
      .json(new ApiResponse(201, deployment, "Deployment started successfully"))
  }
)
...
export const deleteDeployment = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    if (s3Res.Contents && s3Res.Contents.length > 0) {
      ...
      logger.info(
        `🗑️ S3 Cleanup: Deleted ${s3Res.Contents.length} objects for deployment ${id}`
      )
    }
...
export const getProjectById = asyncHandler(
  async (req: Request, res: Response) => {
    ...
    return res
      .status(200)
      .json(
        new ApiResponse(200, result.rows[0], "Project fetched successfully")
      )
  }
)
 ...
 */
export const getDeploymentFiles = asyncHandler(
  async (req: Request, res: Response) => {
    const { id } = req.params
