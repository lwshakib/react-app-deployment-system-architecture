...
async function runCommand(
  ...
  return new Promise((resolve) => {
    ...
    p.on("error", (err) => {
      kafkaService.publishLog(
        `❌ ${stepName} encountered an error: ${err.message}`
      )
      resolve(false)
    })
  })
...
async function init() {
  ...
  try {
    ...
    logger.warn(
      `❌ Kafka connection failed (logs will not be streamed): ${err.message}`
    )
  }
  ...
  if (!fs.existsSync(distFolderPath)) {
    await kafkaService.publishLog(
      `❌ ERROR: No build output folder found (checked dist, build, out)`
    )
    await kafkaService.publishStatus("FAILED")
    await shutdown(1)
    return
  ...
  try {
    ...
    await kafkaService.publishLog(
      `📤 Starting upload to S3 (${distFolderContents.length} files)`
    )
